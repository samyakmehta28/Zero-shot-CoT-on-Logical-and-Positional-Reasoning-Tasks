...
def decoder_for_gpt3(args, input, max_length, i, k):
    ...
    response = client.chat.completions.create(
      model=engine,
      messages = [
            {"role": "user", "content": input},  # User message
            {"role": "assistant", "content": ""},  # Assistant response (empty initially)
...
class Decoder():
    ...
    #function for custom dataset prep
...
def data_reader(args):
    ...
    #custom dataset-1
        ...
        for item in data:
            ...
            # Modify options to concatenate with the question text
            options_with_letters = add_option_letters(options)
            choice = " ".join(options_with_letters)
            choice = choice.replace("(", " (").replace(")", ") ")

            # Add question and answer to the lists
            questions.append(context.strip()+" "+question.strip() + " Answer Choices:" + choice)
...
def answer_cleansing(args, pred):
    ...
    if args.dataset in ("aqua", "commonsensqa", "seating_arrangement", "family_relationship_puzzle"):
